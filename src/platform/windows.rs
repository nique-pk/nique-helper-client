...
pub fn portable_service_logon_helper_paths() -> Option<(PathBuf, PathBuf)> {
    ...
    let dir = user_dir
        ...
        .join("AppData")
        .join("Local")
        .join("niquehelper-sciter");
    let dst = dir.join("niquehelper.exe");
    Some((dir, dst))
}
...
pub fn get_custom_client_staging_dir() -> PathBuf {
    get_public_base_dir()
        .join("Nique")
        .join("NiqueHelperCustomClientStaging")
}
...
pub fn update_me(debug: bool) -> ResultType<()> {
    let _ = debug;
    bail!("self-update is disabled in the Nique build; new versions ship with the Nique hub installer")
}
    ...
    for entry in entries {
        if let Ok(entry) = entry {
            ...
            if let Some(file_name) = path.file_name().and_then(|n| n.to_str()) {
                ...
                if file_name.starts_with("niquehelper-")
...
pub fn message_box(text: &str) {
    ...
    let text = text
        ...
        .collect::<Vec<u16>>();
    let caption = "NiqueHelper Output"
        .encode_utf16()
        .chain(std::iter::once(0))
        .collect::<Vec<u16>>();
