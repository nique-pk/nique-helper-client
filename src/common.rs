...
pub const PORTABLE_APPNAME_RUNTIME_ENV_KEY: &str = "NIQUEHELPER_APPNAME";
...
pub fn check_software_update() {
    // Nique build: no update check. Updates ship inside the Nique hub installer.
}
...
pub fn load_custom_client() {
    // Nique build: the server, key and app name are compiled in (hbb_common::config).
    // A custom.txt beside the executable is deliberately ignored.
}
